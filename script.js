...
import { getFirestore, collection, doc, getDocs, setDoc, updateDoc, addDoc, deleteDoc, query, onSnapshot, orderBy } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            try {
                ...
                document.getElementById('userIdDisplay').textContent = `  ${userId} (Erro de Auth)`;
            }
        }
        isAuthReady = true;
        if (isOnline) {
...
function loadLocalSettings() {
    ...
    currentSetResults = JSON.parse(localStorage.getItem('currentSetResults') || '[]');
...
async function saveLocalSettings() {
    ...
    localStorage.setItem('currentSetResults', JSON.stringify(currentSetResults));

    localStorage.setItem('gameInProgress', JSON.stringify(gameStarted));
...
function setupFirestoreListeners() {
    ...
    onSnapshot(query(collection(db, `artifacts/${appId}/public/data/customTeamNames`), orderBy('order')), (snapshot) => {
        ...
        fetchedCustomTeamNames.forEach(fbTeam => {
            const localTeam = customTeamNames.find(t => t.id === fbTeam.id || t.order === fbTeam.order);
            if (localTeam) {
    ...
    onSnapshot(collection(db, `artifacts/${appId}/public/data/matchHistory`), (snapshot) => {
        const fetchedMatchHistory = [];
        snapshot.forEach(doc => {
            fetchedMatchHistory.push({ id: doc.id, ...doc.data() });
        });

        const newMatchHistory = [];
        fetchedMatchHistory.forEach(fbMatch => {
            const localMatch = matchHistory.find(m => m.id === fbMatch.id);
            if (localMatch) {
                if (fbMatch.lastModified >= localMatch.lastModified) {
                    newMatchHistory.push(fbMatch);
                } else {
                    newMatchHistory.push(localMatch);
                }
            } else {
                newMatchHistory.push(fbMatch);
            }
        ...
        console.error("Erro ao carregar histórico de partidas do Firestore:", error);
        displayMessage("Erro ao carregar histórico de partidas do servidor.", "error");
    });
}
...
async function syncLocalToFirestore() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const fbTeam = firebaseCustomTeamNames.find(t => t.id === localTeam.id || t.order === localTeam.order);
    ...
    const matchHistoryCollectionRef = collection(db, `artifacts/${appId}/public/data/matchHistory`);
    const firebaseMatchHistorySnapshot = await getDocs(matchHistoryCollectionRef);
    const firebaseMatchHistory = firebaseMatchHistorySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

    for (const localMatch of matchHistory) {
        const fbMatch = firebaseMatchHistory.find(m => m.id === localMatch.id);

        if (!fbMatch) {
            if (!localMatch.id) {
                try {
                    const docRef = await addDoc(matchHistoryCollectionRef, localMatch);
                    localMatch.id = docRef.id;
                } catch (e) {
                    console.error(`Erro ao adicionar histórico de partida (ID: ${localMatch.id}) ao Firestore:`, e);
                }
            }
        } else {
            if (localMatch.lastModified > fbMatch.lastModified) {
                try {
                    await updateDoc(doc(matchHistoryCollectionRef, localMatch.id), localMatch);
                } catch (e) {
                    console.error(`Erro ao atualizar histórico de partida (ID: ${localMatch.id}) no Firestore:`, e);
                }
            }
        }
    }
...
function updatePlayerList() {
    ...
    sortedPlayers.forEach((p) => {
        ...
        btn.classList.add("btn", "remove-player-button");
        btn.onclick = () => {
...
function updatePlayerCounter() {
    const selected = players.filter(p => p.checked !== false).length;
...
function confirmReset() {
    ...
    showConfirmationModal("Tem certeza que deseja reiniciar a pontuação? Isso irá zerar os pontos e sets de ambos os times.", async (confirmed) => {
        if (confirmed) {
            ...
            document.getElementById("gearMenu").classList.remove("show");
            document.getElementById("menuOverlay").classList.remove("show");
            showStartGameModal(); // Show the start game modal after reset
        }
    });
}
...
async function resetGame() {
    ...
    // Removed: generatedTeams = []; // Do not clear generatedTeams
    // Removed: renderGeneratedTeams(); // Do not re-render generated teams on reset
    stopAllTimers();
    gameTimeInSeconds = 0;
    setTimeInSeconds = 0;
    updateTimerDisplay();
    document.getElementById('gameTimer').classList.remove('show'); // Hide timer when game is not started
    await saveLocalSettings();
}
...
async function generateCustomTeams() {
    ...
    const selectedPlayers = players.filter(p => p.checked !== false).map(p => p.name);
    ...
    for (let i = 0; i < selectedPlayers.length; i += perTeam) {
        teamCount++;
        const customTeamConfig = customTeamNames[teamCount - 1];
...
function renderGeneratedTeams() {
    ...
    if (generatedTeams.length === 0) {
        ...
        noTeamsMsg.classList.add('no-teams-message');
    ...
    generatedTeams.forEach((team) => {
        ...
        if (team.players.length === 0) {
            ...
            playerLi.classList.add('no-players-message');
...
async function checkWinCondition() {
    ...
    if (winner) {
        ...
        const finalScoreA_for_history = scoreA;
        const finalScoreB_for_history = scoreB;
        let finalSetsA_for_history = setsA;
        let finalSetsB_for_history = setsB;

        currentSetResults.push({
            scoreA: finalScoreA_for_history,
            scoreB: finalScoreB_for_history,
            duration: setTimeInSeconds
        });

        if (winner === 'A') {
            finalSetsA_for_history++;
        } else {
            finalSetsB_for_history++;
        }
        ...
        setTimeout(async () => {
            if (setsToWin > 0 && (finalSetsA_for_history >= setsToWin || finalSetsB_for_history >= setsToWin)) {
                await saveMatchToHistory(
                    winningTeamName,
                    gameTimeInSeconds,
                    finalScoreA_for_history,
                    finalScoreB_for_history,
                    finalSetsA_for_history,
                    finalSetsB_for_history,
                    currentPlayingTeamA,
                    currentPlayingTeamB,
...
function setupSwipeBetweenSections() {
    ...
    document.body.addEventListener('touchend', e => {
        ...
        const sections = ['pontuacao', 'times', 'jogadores', 'agendamentos', 'historico', 'configuracoes'];
...
async function swapTeams() {
    ...
    sectionA.classList.add('pointer-events-none');
    sectionB.classList.add('pointer-events-none');
    document.getElementById('swapTeamsButton').classList.add('pointer-events-none');
    ...
    setTimeout(async () => {
        ...
        setTimeout(() => {
            ...
            sectionA.classList.remove('pointer-events-none');
            sectionB.classList.remove('pointer-events-none');
            document.getElementById('swapTeamsButton').classList.remove('pointer-events-none');
        }, animationDuration);
    }, animationDuration);
}
        ...
        <div id="customConfirmationModal" class="custom-confirmation-modal">
            <div class="modal-content custom-confirmation-content">
                ...
                <div class="modal-buttons">
                    ...
                    <button id="confirmNoButton" class="cancel-button">Não</button>
                </div>
            </div>
        </div>
    `;
    ...
    if (isOnline && isAuthReady) {
        try {
            ...
            for (let i = 0; i < customTeamNames.length; i++) {
                ...
                const docRef = team.id ? doc(customTeamNamesCollectionRef, team.id) : doc(customTeamNamesCollectionRef, `customTeam_${i}`);
...
function updateScoreboardTeamsDisplay() {
    ...
    currentPlayingTeamB.players.forEach(p => {
        const li = document.createElement('li');
        li.textContent = p;
        li.classList.add('inherit-color');
...
async function addAppointment() {
    ...
    const time = gameTimeInput.value.trim();
...
function renderAppointments() {
    ...
    if (!hasUpcoming) {
        upcomingAppointmentsList.innerHTML = '<p class="no-appointments-message">Nenhum agendamento futuro.</p>';
    }
    if (!hasPast) {
        pastAppointmentsList.innerHTML = '<p class="no-appointments-message">Nenhum jogo anterior registrado.</p>';
    }
...
async function saveMatchToHistory(overallWinningTeam, gameDuration, overallScoreA, overallScoreB, setsA, setsB, teamAData, teamBData, setResults) {
    const match = {
        id: null,
        overallWinningTeam: overallWinningTeam,
        overallScoreA: overallScoreA,
        overallScoreB: overallScoreB,
        setsA: setsA,
        setsB: setsB,
    ...
    if (isOnline && isAuthReady) {
        try {
            const docRef = await addDoc(collection(db, `artifacts/${appId}/public/data/matchHistory`), match);
            match.id = docRef.id;
            await saveLocalSettings();
            displayMessage("Histórico de partida salvo e sincronizado!", "success");
        }
        catch (e) {
            console.error("Erro ao salvar histórico de partida no Firestore:", e);
            displayMessage("Erro ao salvar histórico de partida no servidor. Salvo localmente.", "error");
        }
    } else {
        displayMessage("Histórico de partida salvo localmente. Sincronizará quando online.", "info");
    }
    renderMatchHistory();
}
...
function renderMatchHistory() {
    ...
    const noHistoryMessage = document.querySelector('#historico .no-history-message');
    ...
    const sortedHistory = [...matchHistory].sort((a, b) => b.timestamp - a.timestamp);

    sortedHistory.forEach(match => {
        const li = document.createElement('li');
        li.classList.add('history-item');

        const matchDate = new Date(match.timestamp).toLocaleDateString('pt-BR');
        ...
        const formattedGameDuration = `${gameDurationMinutes.toString().padStart(2, '0')}:${gameDurationSeconds.toString().padStart(2, '0')}`;

        const isTeamAWinnerOverall = match.overallWinningTeam === match.teamA.name;
        const isTeamBWinnerOverall = match.overallWinningTeam === match.teamB.name;

        const playersAHtml = match.teamA.players.map(p => `<li class="${isTeamAWinnerOverall ? 'winning-set-score' : ''}">${p}</li>`).join('');
        const playersBHtml = match.teamB.players.map(p => `<li class="${isTeamBWinnerOverall ? 'winning-set-score' : ''}">${p}</li>`).join('');

        const setResultsHtml = match.setResults.map(set => {
            const setDurationMinutes = Math.floor(set.duration / 60);
            const setDurationSeconds = set.duration % 60;
            const formattedSetDuration = `${setDurationMinutes.toString().padStart(2, '0')}:${setDurationSeconds.toString().padStart(2, '0')}`;

            const isTeamAWinningSet = set.scoreA > set.scoreB;
            const isTeamBWinningSet = set.scoreB > set.scoreA;

            return `
                <p>
                    <span class="set-score">
                        <span class="${isTeamAWinningSet ? 'winning-set-score' : ''}">${match.teamA.name} ${set.scoreA}</span>
                        x
                        <span class="${isTeamBWinningSet ? 'winning-set-score' : ''}">${set.scoreB} ${match.teamB.name}</span>
                    </span>
                    <span class="set-duration-time">${formattedSetDuration}</span>
                </p>
            `;
        }).join('');
        ...
        li.innerHTML = `
            <div class="history-card">
                <div class="card-header">
                    <span>
                        <span class="team-names-overall ${isTeamAWinnerOverall ? 'winning-team-name-history' : ''}">${match.teamA.name} 

                        <span>${match.setsA}</span></span>
                        <span class="team-names-overall">x</span>

                        <span class="team-names-overall ${isTeamBWinnerOverall ? 'winning-team-name-history' : ''}"><span class="overall-score">${match.setsB}</span> ${match.teamB.name}</span>
                    </span>
                    <span></span>
                    <div class="date-time-container">
                        <span class="card-date">${matchDate}</span>
                        <span class="card-time">${matchTime}</span>                        
                    </div>
                    <button class="toggle-details-btn" data-id="${match.id}" title="Ver detalhes"><i class="fa-solid fa-chevron-down history-arrow"></i></button>
                </div>
                <div class="history-details" id="details-${match.id}">
                    <div class="details-section players-section">
                        <h4 class="history-card-subtitle">Jogadores:</h4>
                        <div class="players-grid">
                            <div class="details-column">
                                <h4>${match.teamA.name}:</h4>
                                <ul class="player-list-history">
                                    ${playersAHtml}
                                </ul>
                            </div>
                            <div class="details-column">
                    ...
                    <div class="details-section sets-section">
                        <h4 class="history-card-subtitle flexx" style="
    padding-right: 10px;
"><span></span>Sets: <i class="fas fa-clock"></i></h4>
                        
                        <div class="set-results-list">
                            ${setResultsHtml}
                        </div>
                    </div>
                    <div class="details-section location-time-section">
                        <p class="location-info"><i class="fas fa-map-marker-alt"></i> <strong>Local:</strong> ${match.location || 'Não informado'}</p>

                        <span class="flexx">
                            <p class="game-time-info"><i class="fas fa-clock"></i> <strong>Temp de jogo:</strong> ${formattedGameDuration}</p>
                            <button class="delete-history-btn" data-id="${match.id}" title="Excluir histórico"><i class="fas fa-trash-alt"></i></button>
                        </span>
                    </div>
                </div>
    ...
    document.querySelectorAll('.delete-history-btn').forEach(button => {
        button.addEventListener('click', (event) => {
            const idToDelete = event.currentTarget.dataset.id;
            deleteMatchHistoryEntry(idToDelete);
        });
    });

    document.querySelectorAll('.toggle-details-btn').forEach(button => {
        button.addEventListener('click', (event) => {
            const historyCard = event.currentTarget.closest('.history-card');
            if (historyCard) {
                const detailsElement = historyCard.querySelector('.history-details');
                const toggleButtonIcon = event.currentTarget.querySelector('i');

                if (detailsElement && toggleButtonIcon) {
                    detailsElement.classList.toggle('show');
                    toggleButtonIcon.classList.toggle('rotated');
                }
            }
        });
    });
...
async function deleteMatchHistoryEntry(idToDelete) {
    showConfirmationModal("Tem certeza que deseja excluir este registro de histórico?", async (confirmed) => {
        if (confirmed) {
            const initialLength = matchHistory.length;
            matchHistory = matchHistory.filter(match => match.id !== idToDelete);
            if (matchHistory.length < initialLength) {
                ...
                if (isOnline && isAuthReady && idToDelete) {
                    try {
                        await deleteDoc(doc(db, `artifacts/${appId}/public/data/matchHistory`, idToDelete));
                        displayMessage("Registro de histórico excluído e sincronizado!", "success");
                    } catch (e) {
                        displayMessage("Erro ao excluir registro do servidor. Excluído localmente.", "error");
                        console.error("Erro ao excluir registro do Firestore:", e);
                    }
                } else {
                    displayMessage("Registro de histórico excluído localmente. Sincronizará quando online.", "info");
                }
            }
        }
    });
...
async function checkGameContinuation() {
    const gameInProgressFlag = JSON.parse(localStorage.getItem('gameInProgress') || 'false');
    if (gameInProgressFlag && gameStarted) {
        return new Promise(resolve => {
            showConfirmationModal("Foi detectado um jogo em andamento. Deseja continuar de onde parou?", async (confirmed) => {
                if (confirmed) {
                    if (isTimerRunning) {
                        startAllTimers();
                    }
                    ...
                    displayMessage("Iniciando novo jogo.", "info");
                    await resetGame();
                    document.getElementById('startGameButton').classList.remove('hidden');
                    document.getElementById('scoreboardOverlay').classList.remove('hidden');
                }
